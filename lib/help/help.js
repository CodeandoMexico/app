...
page("/help/:page?", valid, parse, function(ctx, next) {
  ...
  if(config['glossary enabled']) glossary.render(content);
...
function valid(ctx, next) {
  ...
  var valid = ['markdown'];
  if (config['faq enabled']) valid.push('faq');
  if (config['tos enabled']) valid.push('terms-of-service');
  if (config['pp enabled']) valid.push('privacy-policy');
  if (config['glossary enabled']) valid.push('glossary');
